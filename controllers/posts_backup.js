		...
		con.query(headerChecker,(err,results)=>{
			...
			if(!err && 
				...
				if(Object.keys(queryObject).length <= 0){
					...
					//select * posts (with count of views, comments, shares, reactions)
				//select user with profile

					let postQuery = "SELECT posts.*, COUNT(comments.uuid) as comments, COUNT(reactions.uuid) as reactions, COUNT(shares.uuid) as shares, COUNT(views.uuid) as views FROM posts LEFT JOIN views ON views.post=posts.uuid LEFT JOIN shares ON shares.post=posts.uuid LEFT JOIN reactions ON reactions.post=posts.uuid LEFT JOIN comments ON comments.ref=posts.uuid GROUP BY posts.id,comments.ref,reactions.post,shares.post";

					con.query(postQuery,(err,post)=>{
						console.log('post'); 
						// console.log(post);

						let user = "SELECT users.*,profiles.* FROM users LEFT JOIN profiles ON users.uuid=profiles.uuid WHERE users.uuid='"+post[0].user+"'";

						console.log("post uuid => " );
						console.log(post[0].uuid);
						...
						post.forEach(
							...
							callback(200,{'post':post,'user':user});
