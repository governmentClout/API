...
handlers._users.get = (data,callback) => {
	...
	let queryString = data.queryStringObject;
...
handlers._profiles.options = (data,callback)=>{
...
handlers._posts.options = (data,callback)=>{
...
handlers._comments.options = (data,callback)=>{
...
handlers._reactions.options = (data,callback)=>{

	callback(200,data.headers);
	
}

handlers._reactions.get = (data,callback)=>{
...
handlers.friends = (data,callback)=>{
	...
	if(acceptableMethods.indexOf(data.method) > -1){
		...
		handlers._friends[data.method](data,callback);
...
handlers._friends.options = (data,callback)=>{

	callback(200,data.headers);
	
}

handlers._friends.get = (data,callback)=>{
			...
			if(
				...
				result[0].token == token 
					...
					if(post){
						...
						let sql = "INSERT INTO friends (user,friend) VALUES('"+uuid+"','" + post+"')";
