			...
			if(
				...
				result[0].token == token 
					...
					if(post){
						...
						let sql = "INSERT INTO posts (post,location,attachment,uuid,user,type) VALUES('"+post+"','" + location+"','"+attachment+"','"+uuid+"','"+user+"','"+post_type+"')";
	...
	if(data && 
		...
		con.query(headerChecker,(err,results)=>{
			...
			if(!err && 
				...
				let postQuery = "SELECT posts.post,posts.attachment,posts.location,posts.uuid,posts.created_at,posts.updated_at,comments.comment,comments.created_at,comments.updated_at,comments.user FROM posts,comments WHERE posts.uuid='" + post + "' AND comments.ref='"+uuid+"'";
			// console.log('uuid ' + uuidHeader);
		...
		con.query(verifyToken, (err,result)=>{
			...
			if(
				...
				result[0].token == token 
					...
					let checkPost = "SELECT * FROM posts WHERE uuid='" + uuid + "'";
					...
					con.query(checkPost, (err,result)=>{
						...
						if(!err && 
							...
							let sql = "UPDATE posts SET post='" + post + "', location ='"+location+"', attachment ='"+attachment+"', updated_at='"+updated_at.toString()+"' WHERE uuid='" +uuid+ "'"; 
...
handlers._reactions.post = (data,callback)=>{
	callback(200,{'Success':'You have hit the reaction post endpoint'});
}

handlers._reactions.delete = (data,callback)=>{
	callback(200,{'Success':'You have hit the reaction delete endpoint'});
}
