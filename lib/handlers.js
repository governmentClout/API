	...
	if(data && 
		...
		con.query(headerChecker,(err,results)=>{
			...
			if(!err && 
				...
				let post = "SELECT post,attachment,location,uuid,created_at,updated_at FROM posts WHERE uuid='" + uuid + "'";
			// console.log('uuid ' + uuidHeader);
	...
	if(data && 
		...
		con.query(headerChecker,(err,results)=>{
			...
			if(!err && 
				...
				let post = "SELECT count(*) FROM posts WHERE uuid='" + uuid + "'";
			
				con.query(post,(err,result)=>{

					if(!err && result[0]){

						let deletePost = "DELETE FROM posts WHERE uuid='"+uuid+"'";

						con.query(deletPost,(err,result)=>{
