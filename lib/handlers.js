	...
	if(data && 
		...
		con.query(headerChecker,(err,results)=>{
			...
			if(!err && 
				...
				let postQuery = "SELECT posts.post,posts.attachment,posts.type,posts.location,posts.uuid,posts.created_at,posts.updated_at,comments.comment,comments.created_at,comments.updated_at,comments.user FROM posts,comments WHERE posts.uuid = comments.ref";
			// console.log('uuid ' + uuidHeader);
				...
				result[0].token == token 
					...
					con.query(checkPost, (err,result)=>{
						...
						if(!err && 
							...
							let sql = "UPDATE posts SET post='" + post + "', location ='"+location+"', attachment ='"+attachment+"','"+post_type+"', updated_at='"+updated_at.toString()+"' WHERE uuid='" +uuid+ "'"; 
